...
def main(conn):
    ...
    weather_clean_quoted = quote_identifier(TABLE_WEATHER_CLEAN)
    column_type_rows = conn.execute(
        f"""
        SELECT column_name, data_type
        FROM information_schema.columns
        WHERE table_name = '{TABLE_WEATHER_CLEAN}'
    ...
    total_rows = conn.execute(
        f"SELECT count(*) FROM {weather_clean_quoted}"
    ).fetchone()[0]

    print(f"Input table: {TABLE_WEATHER_CLEAN}")
    ...
    report = {
        ...
        "input_table": TABLE_WEATHER_CLEAN,
