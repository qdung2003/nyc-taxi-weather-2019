...
file_02 = taxi_root / "eda" / "results" / "02_check_duplicate.json"
file_03 = taxi_root / "eda" / "results" / "03_view_high_duplicate.json"
output_file = taxi_root / "eda" / "results" / "04_all_columns_profile.html"
...
def pct(count, total):
    if not total:
        return 0.0
    return round(float(count) / float(total) * 100, 5)


def trim_low_distribution(low_obj):
    values = low_obj.get("values", [])
    counts = low_obj.get("quantity", [])
    percents = low_obj.get("quantity_percent", [])
...
def build_month_rows(profile_03_col, total_rows):
    ...
    for i, label in enumerate(MONTH_LABELS):
        ...
        rows.append({"label": label, "count": count, "percent": pct(count, total_rows)})
    return rows
...
def build_range_rows_from_eda03(profile_03_col, total_rows):
    ...
    rows = [
        {"label": "< 0", "x_label": "<0", "count": negative, "percent": pct(negative, total_rows)},
        {"label": "= 0", "x_label": "0", "count": zero, "percent": pct(zero, total_rows)},
    ]
    ...
    rows.append(
        {
            ...
            "percent": pct(above_max_chart_quantity, total_rows),
        }
    )
...
def build_merged_columns(profile_02, profile_03):
    ...
    for col_name, schema_type in schema_map.items():
        ...
        if high03 and str(high03.get("type_value", "")).startswith("timestamp"):
        ...
        merged.append(
            {
                ...
                "month_bucket_count": len(chart_rows) if (high03 and str(high03.get("type_value", "")).startswith("timestamp")) else None,
...
<body>
  ...
  <script>
    ...
    function renderChart(c) {{
      ...
      const W = 1040, H = 252;
      // Keep only ~10px free space below x-label area
      ...
      const yScaleMax = maxY * 1.12; // add top headroom so tallest bar can show percent above
      for (let i = 0; i <= 5; i++) {{
