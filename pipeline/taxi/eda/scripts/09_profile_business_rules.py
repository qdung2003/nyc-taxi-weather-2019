...
from pipeline.services.queries import connect_and_check, get_schema, get_total_rows
...
output_file = output_dir / "09_profile_business_rules.json"
...
def main(conn):
    source_table = "tmp_eda09_source"
    conn.execute(f'DROP TABLE IF EXISTS "{source_table}"')
    conn.execute(
        f'CREATE TEMP TABLE "{source_table}" AS SELECT * FROM {quote_identifier(VIEW_TAXI_BUSINESS_RULES)}'
    )
    ...
    for field in tqdm(
        ...
        if type_text.startswith("timestamp"):
            ...
            note = "12-month distribution from v_taxi_03_business_rules"
        elif col in CONTINUOUS_COLUMNS:
            chart_rows, range_bucket_count = build_continuous_rows(conn, source_table, col, total_rows)
            note = "Zero and positive bins from v_taxi_03_business_rules (negative bins hidden; 10 raw bins)"
        else:
            chart_rows = build_exact_rows(conn, source_table, col, total_rows)
            unique_count = len(chart_rows)
            note = "Value distribution from v_taxi_03_business_rules (count=0 values hidden)"
    ...
    payload = {
        "summary": {
            ...
            "clean_flow": VIEW_TAXI_BUSINESS_RULES,
        },
