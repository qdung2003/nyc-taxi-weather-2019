...
function isSchemaTypeMatch(referenceType, databaseType) {
  const ref = normalizeTypeName(referenceType);
  const db = normalizeTypeName(databaseType);
  if (!ref || !db) return false;

  const refToDbCandidates = {
    INT64: new Set(['BIGINT', 'INTEGER', 'INT', 'INT8']),
    DOUBLE: new Set(['DOUBLE', 'FLOAT', 'REAL', 'DECIMAL', 'NUMERIC']),
    STRING: new Set(['VARCHAR', 'TEXT', 'STRING']),
    'DATE32[DAY]': new Set(['DATE']),
    'TIMESTAMP[US]': new Set(['TIMESTAMP', 'DATETIME']),
    BOOLEAN: new Set(['BOOLEAN', 'BOOL']),
  };

  const candidates = refToDbCandidates[ref];
  if (candidates) {
    if (candidates.has(db)) return true;
    for (const c of candidates) {
      if (db.startsWith(`${c}(`)) return true;
    }
    return false;
  }
...
function renderScalarChips(chips, item) {
  ...
  const entries = Object.entries(item)
    .filter(([key, value]) => key !== 'column_name' && (
...
function renderPrimitiveKpis(root, data, step) {
  ...
  if (['02', '03', '05', '06', '07', '08'].includes(String(step))) {
    kpis.classList.add('kpis-inline', 'kpis-eda01-style');
  }
  const primitiveEntries = Object.entries(data).filter(([, v]) => (
  ...
  const preferred = preferredByStep[String(step || '')] || [];
...
function buildLowDistributionRows(col) {
  ...
  const isLocationCol = colName === 'PULocationID' || colName === 'DOLocationID';
...
function buildProfileRows(col, totalRows) {
  if (col.month_counts && col.month_percentages) return buildEda03MonthRows(col, totalRows);
  if (col.bin_edges && col.bin_counts && col.bin_percentages) return buildEda03RangeRows(col, totalRows);
  if (col.chart_rows) return toSeriesRows(col.chart_rows);
  if (col.values && col.counts && col.percentages) return buildLowDistributionRows(col);
  return toSeriesRows({
...
function buildProfileRowsForMode(col, totalRows, mode) {
  if (['06', '08'].includes(String(mode))) {
    ...
    if (col.column_name === 'PULocationID' || col.column_name === 'DOLocationID') return toSeriesRows(buildLowDistributionRows(col));
    return toSeriesRows(buildFullDistributionRows(col));
  }
  if (mode === '02') return toSeriesRows(buildLowDistributionRows(col));
  return toSeriesRows(buildProfileRows(col, totalRows));
}
